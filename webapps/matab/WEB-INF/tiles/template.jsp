...
<!DOCTYPE html>
<%--<html style="will-change:transform,opacity">--%>
<html>
<head>
                                	...
                                	<tr>
                                		...
                                		<td width="80" align="center">
                                			<table>
                                				...
                                				<tr>
                                					<td align="center">
                                                        <h1><%=GlobalVariable.getUser().getUserName()%></h61>
                                					</td>
                                				</tr>
                                			</table>
                                		</td>
							...
							<tr>
								<td>
									<table>
										<tr>
											<td>
												<label for="${name}"><%=ResourceBoundleHolder.resourceBundle.getString("login.oldPassword")%></label>
											</td>
											<td>
										...
										<tr>
											<td>
												<label for="${name}"><%=ResourceBoundleHolder.resourceBundle.getString("login.newPassword")%></label>
										...
										<tr>
											<td>
												<label for="${name}"><%=ResourceBoundleHolder.resourceBundle.getString("login.confirmPassword")%></label>
											</td>
											<td>
...
////////////////
		    $("#confirmChangePassBTN").click(function(){
		    	var dataSend = {};
                dataSend['oldPass']=document.getElementById('oldPassword').value;
                dataSend['newPass']=document.getElementById('newPassword').value;
                dataSend['confirmPass']=document.getElementById('confirmPassword').value;
		    
	            jQuery.ajax({
	                url:"<c:url value='/' />" + "system/authentication/user/changePassword/",
                    type:"POST",
	                ...
	                success:function (result) {
                        ...
                        if (result.state == "success") {
                            ...
                            $("#changePassWND").data("kendoWindow").close();
                        }
					}
	            });
            });
...
///////////////
		    ...
		    if(window.location.href.indexOf("/editWP/")  > 0){
			 ...
			 var defaultPageController = "<%=GlobalVariable.getUser().getDefaultPageController()%>";
			 if(defaultPageController != "null" && defaultPageController.trim() != ""){
				 document.getElementById('myIframe').src =  "${pageContext.request.contextPath}/" + defaultPageController;
			 }
...
//                alert("error andar error!");
		...
		function onSelect(e){
			var fullScreenMode = <%=GlobalVariable.getUser().getFullScreenMode()%>;
			...
			//console.log(action);
			if(action=='j_spring_security_logout'){
				window.location.href = "${pageContext.request.contextPath}/" + action;
			}else if(action!=undefined){
				//window.location.href = "${pageContext.request.contextPath}/" + action;
				console.log(action);
				document.getElementById('myIframe').src =  "${pageContext.request.contextPath}/" + action;
				//$('#myIframe').effect('shakeFrame');
				//$("#myIframe").hide();
				//$("#loadLink").click(function() {
				//	$("#myIframe").hide();
				//	$("#myIframe").attr('src', 'http://mysite.com/newpage.html');
				//});
...
<script>
		...
		function myTimer(){
			...
			//window.alert(t);
			document.getElementById("demo").innerHTML=ToShamsi(d.getFullYear(),d.getMonth()+1,d.getDate(),"Long");
			//document.getElementById("demo1").innerHTML="<%=GlobalVariable.getUser().getUserName() %>";
			/*document.getElementById("demo").innerHTML=(
			(d.getHours()<10)?"0"+d.getHours():d.getHours())+":"+
			((d.getMinutes()<10)?"0"+d.getMinutes():d.getMinutes())+":"+
			((d.getSeconds()<10)?"0"+d.getSeconds():d.getSeconds())+"    "+
			ToShamsi(d.getFullYear(),d.getMonth()+1,d.getDate(),"Long")+" "+d.toDateString();*/
		}
</script>
...
function handleClick(myRadio) {
	var requestUrl = '${pageContext.request.contextPath}'+'/fnc/generalAccounting/basicinfo/financeYear/activate/'+myRadio.value;
