...
<div class="uk-container uk-container-center my-uk-container" >
	<form class="uk-form" id="mainForm">
		...
		<div class="uk-grid uk-grid-preserve">
			<base:label  name="countPurchaseContractOperation" styleClass="verysmall" key="com.artonis.inv.m169.reports.count"></base:label>
		...
		<div class="uk-grid uk-grid-preserve">
			...
			<div class="uk-grid uk-grid-preserve">
			<base:label  name="deductionTax" styleClass="verysmall" key="com.artonis.inv.m169.reports.deduction"></base:label>
			<base:label  name="backingDeductionTax" styleClass="verysmall" key="com.artonis.inv.m169.reports.backingDeduction"></base:label>	
		</div>

	 	<div class="uk-grid uk-grid-preserve">
	 	  <base:comboBox styleClass="small" name="personType" ignore="true" isListConstant="true"
                           key="com.artonis.inv.m169.basicInfo.companyPerson.personType" comboLoaderUrl="/combo/loadAllPersonType" />
  	 		<base:comboBox styleClass="small" name="companyType" ignore="true" isListConstant="true"
     ...
     <div id="header-toolbar" align="left">
     	  	<base:buttonPermission key="fnc.btn.registerInfo" classNames="uk-button" id="purchaseContractReportInput-registerInfo-btn" onClick="javascript:savePurchaseContractReport()"  style="width: 100px; color: blue;"/>
		  	<base:buttonPermission key="fnc.btn.print" classNames="uk-button" id="purchaseContractReportInput-print-btn" onClick="javascript:myPrint()" style="width: 100px; color: green;"/>
		  	<base:buttonPermission key="fnc.btn.cancel" classNames="uk-button" id="purchaseContractReportInput-cancel-btn" onClick="javascript:showPurchaseContractReportView()" style="width: 100px; color: green;"/>
    </div>
</form>
</div>
...
$(document).ready(function(){
	...
	$.getJSON(url, function (data) {
		...
		$('#countPurchaseContractOperation').val(data.countPurchaseContractOperation);
		$('#toll').val(data.toll);
		$('#tax').val(data.tax);
		...
		$('#backingDeductionTax').val(data.backingDeductionTax);
 ...
 function savePurchaseContractReport(){
  save();
 }
   function callback_DataBound_personType(e){
    if(document.getElementsByName('personType')[0].value !="" && document.getElementsByName('companyType')[0].value!=""){
   loadCompanyPerson(document.getElementsByName('personType')[0].value,document.getElementsByName('companyType')[0].value);
   changeTextByCompanyType();
 }
}
function showPurchaseContractReportView(){
	window.location = "${pageContext.request.contextPath}"+"/inventory/m169/reports/purchaseContractReport/";
}
function getPageObject(){
		...
		param.deductionTax = $("#deductionTax").val();
		param.backingDeductionTax = $("#backingDeductionTax").val();
		return param;
}
function myPrint(){
	...
	jsonString = jsonString.replace(/\\/g, ':@:');
	window.location = "${pageContext.request.contextPath}"+"/inventory/m169/reports/purchaseContractReport/print/" + jsonString;
}
</script>
