...
<div class="uk-container uk-container-center my-uk-container" >
	...
	<div class="uk-grid uk-grid-preserve">
	        <base:comboBox styleClass="verySmall" name="accountLayer" key="com.artonis.fnc.reports.balance.floatAccountBalance.floatAccountLayer" comboLoaderUrl="combo/loadAllFloatAccountLayer"/>
	...
	<div class="uk-grid uk-grid-preserve">
				<base:comboBox styleClass="verySmall" name="floatAccountChoice"  key="com.artonis.fnc.reports.balance.floatAccountBalance.floatAccountChoice" comboLoaderUrl="combo/loadAllBalanceAccountChoice"/>
	</div>
	<div class="uk-grid uk-grid-preserve">
	</div>
	<div id="floatAccountsListDiv">
	<div class="uk-grid uk-grid-preserve" >
	   <base:comboBoxAutoComplete-buttonAddToCombo name="floatAccount_Id"  key="com.artonis.fnc.reports.books.accountBook.floatAccountCodeName"  popupUrlIsRelative="true"
	       ...
	       <button class="uk-button uk-button-success k-rtl" type="button" onclick="javascript:addToFloatAccountsList()" id="addBtn">
   ...
   <div style='width: 82%' class="uk-width-4-10 k-rtl"  id="${name}_Div">
		 <select id="floatAccountsList"  multiple="multiple" class="uk-width-4-10 k-rtl">
         </select>
    </div>
   </div>                       
    <div class="uk-grid uk-grid-preserve"  >
   ...
   <div style='width: 82%' class="uk-width-4-10 k-rtl"  >
		  <button class="uk-button uk-button-success" type="button" onclick="javascript:deleteFloatItem()" id="deleteBtn">
		      <%=ResourceBoundleHolder.resourceBundle.getString("global.delete")%>
	       </button>
	         <button class="uk-button uk-button-success" type="button" onclick="javascript:deleteAllFloatItems()" id="deleteAllBtn">
   ...
   <div style='width: 82%' class="uk-width-4-10 k-rtl"  >
     ...
     <div style="text-align: center">
		 <base:buttonPopupGrid popUpGridName="grid"  customPopupUrlTailFunctionName="customizeUrlFunctionName" isRelative="true" popUpGridUrl="item/FloatAccountBalance" formPopup="FloatAccountBalanceItem" popUrl="item/FloatAccountBalance" key="global.search"></base:buttonPopupGrid>
...
<script type="text/javascript">
	...
	testBalanceVar.floatAccountChoice=$("#floatAccountChoice").val();
	testBalanceVar.accountsList=allAccountsInListSeperateWithComma();
	testBalanceVar.floatAccountsList=allFloatAccountsInListSeperateWithComma();
	return testBalanceVar;
}
   $(document).ready(function() {
     $("#accountLayer").val(1);
     $("#accountsListDiv").hide();
     $("#floatAccountsListDiv").hide();
     $("#showType").val(3); //default for 6column
     $("#accountChoice").val(1); //default for AllAccount
     $("#floatAccountChoice").val(1); //default for AllAccount
    });
...
function clearAllFields(){
    ...
    function addToAccountsList(){
      ...
      if($("#accountCoding_IdStr_Comb").val() != ""){
        ...
        option.value=$("#accountCoding_IdStr_Comb").val();
        x.add(option);
   ...
   if(accountChoiceVal== 2)
    ...
    function callback_change_floatAccountChoice(e){
   var accountChoiceVal = $("#floatAccountChoice_Comb").data("kendoComboBox")._selectedValue;
   if(accountChoiceVal== 2)
      $("#floatAccountsListDiv").slideToggle(600);
       if(accountChoiceVal== 1){
      $("#floatAccountsListDiv").hide(600);
       
       }
   }
   
    function addToFloatAccountsList(){
        var x = document.getElementById("floatAccountsList");
        var option = document.createElement("option");
       if($("#floatAccount_Id_Comb").val() != ""){
        option.text = $("#floatAccount_Id_Comb").data("kendoComboBox").text();
        option.value=$("#floatAccount_Id_Comb").val();
        x.add(option);
        $("#floatAccount_Id_Comb").val("");
        $("#floatAccount_Id_Comb").data("kendoComboBox").value("");
        $("#floatAccount_Id_Comb").data("kendoComboBox").dataSource.filter([]);
      } 
    }
    function deleteFloatItem(){
     $("#floatAccountsList option:selected").remove();
    }
    function deleteAllFloatItems(){
    $('#floatAccountsList').find('option').remove().end()
    }
   function callback_change_accountChoice(e){
   var accountChoiceVal = $("#accountChoice_Comb").data("kendoComboBox")._selectedValue;
   if(accountChoiceVal== 2)
   ...
   function allFloatAccountsInListSeperateWithComma(){
   var options = $('#floatAccountsList option');
   var values = $.map(options ,function(option) {
    return option.value;
      });
     return values;
   ...
   function _grid_onDataBound(e){
    ...
    function setSumBoxes(){
        ...
        searchJsonString = JSON.stringify(getSearchObject());
        searchJsonString = searchJsonString.replace(/\//g, ':@:');
        var requestUrl = '${pageContext.request.contextPath}'+'/fnc/reports/balance/floatAccountBalance/setSumBoxes/'+searchJsonString;
