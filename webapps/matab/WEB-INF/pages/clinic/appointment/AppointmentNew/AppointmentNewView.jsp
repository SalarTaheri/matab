...
<body>
<div id="tableheadr" >
	<table width="100%" style="table-layout: fixed; color: #000000; background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#deefff), color-stop(100%,#98bede)); ">
		<tr style="width: 100%">
			<td width="10%">
			...
			<td width="20%">
	       		...
	       		<input type="text" name="nameAndFamForAppointment" id="nameAndFamForAppointment" class="text ui-widget-content ui-corner-all" style=" width: 80%; ">
			</td>
			<td width="8%" align="left">
				<input type="text" id="simpleText" placeholder="انتخاب تاریخ" style="width: 100%" autocomplete="off"/>
			</td>
			<td width="12%" align="center">
				<base:simpleLabel name="showCancelCheckbox" key="com.appointmentNew.showCancelState"/>
				<input type="checkbox" id="showCancelCheckbox" onchange="showCancelCheckboxChange()" />
			</td>
			<td width="10%" >
                ...
                <button class="button_example" id="btntoday" onclick="toDayDate()"><%=ResourceBoundleHolder.resourceBundle.getString("com.appointment.today")%></button>
            </td>
		</tr>
	</table>
</div>
...
<div id="dialog-form" class="toPopup" title='<%=ResourceBoundleHolder.resourceBundle.getString("com.appointment.appintmentTime")%>' style="display:none">
    <form id="sabt-nobat" class="toPopup">
        <table  border="0" width="100%">
            ...
            <tr >
                ...
                <th width="60%">
                	<base:optiion  absulotePath="clinic/appointment/appointment/combo/loadServiceAsCombo/-1/-1"
                                   name="actionListName" key=""
                                   noneSelectedText='<%=ResourceBoundleHolder.resourceBundle.getString("com.appointment.operationSelection")%>'/>
                </th>
            </tr>
            <tr>
               	<th id="labelCol" width="40%"  >
            ...
            <%if(GlobalVariable.getButtonContainer("005011007", "oldFileNumberVisibility")){%>
	            <tr>
	                <th id="labelCol" width="40%" >
	                    <input type="hidden" id="patientId">
	                    <base:simpleLabel name="oldFileNumber" key="com.appointment.oldFileNumber"/>
	                </th>
	                <th width="60%">
	                    <input disabled="disabled" type="text" name="oldFileNumber" id="oldFileNumber" class="text ui-widget-content ui-corner-all" style=" width: 100%; ">
	                </th>
	            </tr>
            <%}%>
            <tr>
        ...
        buttons: {
            "قرار ویزیت": function() {
                ...
                if ( bValid ) {
                    ...
                    data.timeInDayId = globalThAddress;//
                    data.tdTime = globalTdTime;
                    var form1=findPopupForm();
                    data.nameAndFam = form1.nameAndFam.value;
                    data.fileNumber = form1.fileNumber.value;
                    if(form1.oldFileNumber != undefined){
                    	data.oldFileNumber = form1.oldFileNumber.value;
                    }
                    data.patientId = form1.patientId.value;
                    ...
                    if(globalAppointmentDate==""){
                    	var monthMM=((appGlobalDate.getMonth()+1)<10)?"0"+(appGlobalDate.getMonth()+1):(appGlobalDate.getMonth()+1);
                    	var dayDD=((appGlobalDate.getDate())<10)?"0"+(appGlobalDate.getDate()):(appGlobalDate.getDate());
                    	data.appDate =appGlobalDate.getFullYear()+"-"+monthMM+"-"+dayDD;
                    }else{
                    	var monthMM=globalAppointmentDate.split("-")[1];
                    	if((globalAppointmentDate.split("-")[1]).length<2){
                    		monthMM="0"+(globalAppointmentDate.split("-")[1]);
						}
                    	
                    	var dayDD=globalAppointmentDate.split("-")[2];
                    	if((globalAppointmentDate.split("-")[2]).length<2){
                    		dayDD="0"+(globalAppointmentDate.split("-")[2]);
                    	}
                    	
                    	data.appDate =globalAppointmentDate.split("-")[0]+"-"+monthMM+"-"+dayDD;
                    }
                    data.phone=form1.phone.value;
                    data.cellPhone=form1.cellPhone.value;
                    data.description=form1.description.value;
                    //data.patientId = 1;

                    if($("#actionListName").val()==null || form1.nameAndFam.value==""){
                    	window.alert("لطفا آیتم های عمل را انتخاب کنید");
                    	return;
                    }
                    if(data.cellPhone!="" && (data.cellPhone.length != 11 || data.cellPhone.indexOf('09')==-1)){
                    	window.alert("تلفن همراه نامعتبر می باشد");
                    	return;
                    }
 
                    $.ajax({
                        url: "<c:url value='/' />" + "clinic/appointment/appointmentNew/saveAppointment/"  ,
                        type:"POST",
                        ...
                        success: function (data) {
                            if(data.state=="success"){
                                var form1=findPopupForm();
                                state = (form1.patientId.value=="") ? 1 : 2 ;
                                
                                makeCardAndAdd(data.id, globalThAddress, globalAppointmentDate, form1.patientId.value, form1.fileNumber.value,
                                		form1.oldFileNumber.value, form1.nameAndFam.value, form1.phone.value,
                                		form1.cellPhone.value, state, null, null, $(".ui-multiselect" ).text(), $("#actionListName").val(),
                                		$("#doctorCombo").val(), $("#doctorCombo option:selected").text(), form1.description.value, 1);
                                
                                form1.nameAndFam.value="";
                                form1.fileNumber.value="";
                                if(form1.oldFileNumber != undefined){
                                	form1.oldFileNumber.value="";
                                }
                                form1.phone.value="";
                                form1.cellPhone.value="";
                                form1.patientId.value="";
...
//	  		appGlobalDate.setDate(gDate[2]);
          appGlobalDate=new Date(gDate[0],gDate[1]-1,gDate[2]);
...
$( "#nameAndFam" ).autocomplete({
            ...
            success: function (data) {
                response( $.map( data, function( item ) {
                    return {
                        ...
                        id: item.fileNumber+';'+item.ac_id+';'+item.tel1+';'+item.cellPhon+';'+item.oldFileNumber,
    ...
    select: function( event, ui ) {
        ...
        if(allData[4]!='undefined'){
            $('#oldFileNumber').val(allData[4]);
        }
    },  open: function(event, ui) {
//        $(".ui-autocomplete").css("position", "absolute");
//        $(".ui-autocomplete").css("top", "100px");
...
function findPopupForm(){
    for(var i=0;i<document.forms.length;i++){
        if(document.forms[i].id=="sabt-nobat"){
...
function reLoadOnDepartmentCombo(){
	...
	$.ajax({
            url: "${pageContext.request.contextPath}"+"/clinic/appointment/appointmentNew/combo/loadAllDoctor/"+$("#departmentCombo").val(),
            type:"GET",
            dataType:"json",
            contentType:"application/json; charset=utf-8",
            success: function (data) {
...
var allowEditNew = false; //use for check edit and insert availability
function reLoadOnDoctorCombo(){
	$.getJSON("<c:url value='/' />" + 'clinic/appointment/appointment/combo/loadServiceAsCombo/'+$("#departmentCombo").val()+'/'+$("#doctorCombo").val() , function(data) {
 	...
 	$.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadAllowEditNewValue/" + $("#departmentCombo").val() + "/" + $("#doctorCombo").val(), function(data) {
 		allowEditNew = data.allowEditNewValue;
		if(weekViewFlg){
       		weekView(appGlobalDate);
    	}else{
	    	...
	    	loadCurrentData(dayForSearch,dayForSearch,appGlobalDate);
    	}
 	});
}

var addCardIdForShow;
function reLoadAfterChange(cardId){
        addCardIdForShow=cardId;
	    dayForSearch=appGlobalDate.getFullYear()+"-"+(appGlobalDate.getMonth()+1)+"-"+appGlobalDate.getDate();
	    autoCompleteForSearchOnAppointment(dayForSearch);
	    deleteRow('appTable');
	    //makeEmptyTable(dayForSearch,appGlobalDate);
...
function loadDataForWeekView(dateForSearchInput,cellIdx){
    $.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadAppontment/" + dateForSearchInput+"/"+$("#doctorCombo").val()+"/"+$("#departmentCombo").val() , function(data) {
        var nEmptyList=true;
        try{
        ...
        var rwCounter=data.length;
        for(i=1;i<=rwCounter;i++){
            if(nEmptyList){
                for ( var j =0; j<data.length;j++) {
                        makeCardAndAddForWeek("wv_c_"+cellIdx+"_"+i,data[j].startTime,data[j].id,data[j].state);
                }
            }
        }
...
function loadDataForWeekViewCount(dateForSearchInput,cellIdx){
    ...
    $.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadAppontmentCount/" + dateForSearchInput+"/"+$("#doctorCombo").val()+"/"+$("#departmentCombo").val() , function(data) {
...
function loadCurrentData(dateForSearchInput,dateTable,date){
    $.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadAppontment/" + dateForSearchInput +"/"+$("#doctorCombo").val()+"/"+$("#departmentCombo").val(), function(data) {
        var nEmptyList=true;
        try{
            nEmptyList=!data[0].empty;
    ...
    var admissionDayString = '<%=ResourceBoundleHolder.resourceBundle.getString("com.artonis.clinic.appointment.appointmentNew.admissionDay")%>';
    ...
    for(i=0;i<data.length;i++){
        ...
        td1.style.width="10%";
        var newCellId="c2_"+data[i].id;;
        if(i==0){
            	sortableStr="#c2_"+data[i].id;
        }else{
            sortableStr+=(",#c2_"+data[i].id);
        }
 		td1.appendChild(document.createTextNode(data[i].startTime));
        td1.id="c1_"+i;
	    ...
	    if(data[i].startTime!=admissionDayString){  
	        today=new Date();
	        ...
	        if(compare!=-1){
	            td2.onclick = function () {
	                var idAddress= new Array();
		        		...
		        		tmp_adr='';
		                addCard(this.id.substring(3, this.id.length),dateTable,tmp_adr);
	            };
	        }
	    }
    ...
    $( sortableStr ).sortable({
        ...
        ,start: function(e, ui) {
        	console.log(($(this).attr('id')).substring(3, this.id.length));
        ...
        ,stop: function(e, ui) {
            ...
            var countOfItems = $("#"+ui.item[0].id).parent().contents().length;
            crntdayForChange = tmpDate.getFullYear()+"-"+(tmpDate.getMonth()+1)+"-"+tmpDate.getDate();
            var data = {};
            data.startPosition = ++startPosition;
            data.endPosition = ++endPosition;
            data.countOfItems = countOfItems;
            data.appointmentId = ui.item[0].id;//
            data.tdId = ($("#"+ui.item[0].id).parent().attr('id')).substring(3, this.id.length);
            data.appointmentDate = crntdayForChange;
            $.ajax({
                url: "<c:url value='/' />" + "clinic/appointment/appointmentNew/changeAppointmentTime/"  ,
                type:"POST",
                data:JSON.stringify(data),
                dataType:"json",
                contentType:"application/json; charset=utf-8",
                success: function (data) {
                     ...
                     loadCurrentData(yesterdayForSearch,yesterdayForSearch,yesterday);
					if(data.state == "error"){
        ...
        var rowIds="-1";
        for(i=0;i<data.length;i++){
        	rowIds=rowIds+','+data[i].id;
        }
        findAppointment(rowIds);
...
function findAppointment(timeIndayIds){
var showCancel = $('#showCancelCheckbox')[0].checked;
 $.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadPatientAppontment/" + timeIndayIds + "/" + showCancel, function(dayAppointmentData) {
     for ( var i =0; i<dayAppointmentData.length;i++) {
             makeCardAndAdd(dayAppointmentData[i].id,
					dayAppointmentData[i].timeInDayId,
                    dayAppointmentData[i].patientId,
                    dayAppointmentData[i].fileNumber,
                    dayAppointmentData[i].oldFileNumber,
                    ...
                    dayAppointmentData[i].registerUser,
                    dayAppointmentData[i].factor_id);
    }
     if(addCardIdForShow!=undefined && addCardIdForShow!="-1"){
         $('html,body').animate({
                     scrollTop: $("#"+addCardIdForShow).offset().top},
                 'slow');
         addCardIdForShow="-1";
     }
...
function patientSpecView(nameAndFam,fileNumber,oldFileNumber,phone,cellPhone){
    if(typeof(fileNumber) == "undefined"){
        fileNumber="  ";
    }
    if(typeof(oldFileNumber) == "undefined"){
    	oldFileNumber=" ";
    }
...
//        var spec="نام و نام خانوادگی :"+nameAndFam+"<br>";
    ...
    spec+="   شماره پرونده قدیم:"+oldFileNumber+"&nbsp&nbsp";
...
function addCard(th1,appointmentDate,tdTime){
	...
	globalTdTime=tdTime;
 	...
 	var doctorId = $("#doctorCombo").val();
 	var departmentId = $("#departmentCombo").val();
 	
	if(allowEditNew == true){
...
function makeCardAndAdd(patientAppointmentId,timeInDayId,pntPatientId,pntFileNumber,pntOldFileNumber,patientNF,phone,cellPhone,
		appointmentState,actionListNames,actionListIds,doctorName,description,registerUser,factorId){
    ...
    }else if(appointmentState==8){
        aTag_visit.appendChild(document.createTextNode("انصراف داده شده"));
    }
    ...
    if(registerUser!="Internet"){
    	...
    	aTag_edit.onclick= function (event) {
    		globalThAddress=timeInDayId;
            var form1=findPopupForm();
            form1.nameAndFam.value=patientNF;
    		form1.fileNumber.value=pntFileNumber;
    		form1.actionListName.value=pntFileNumber;
    		if(form1.oldFileNumber != undefined){
	    		if(pntOldFileNumber == undefined){
	   				form1.oldFileNumber.value="";
	    		}else{
	    			form1.oldFileNumber.value=pntOldFileNumber;
	    		}
    		}
    		$("#doctorTitleId").text(doctorName);
            if(phone!='undefined'){
                form1.phone.value=phone;
            }
            if(cellPhone!='undefined'){
                form1.cellPhone.value=cellPhone;
            }
            ...
            globalAppointmentId=patientAppointmentId;
        	$( "#dialog-form" ).dialog( "open" );
            if ($.ui && $.ui.dialog && $.ui.dialog.prototype._allowInteraction) {
                var ui_dialog_interaction = $.ui.dialog.prototype._allowInteraction;
                $.ui.dialog.prototype._allowInteraction = function(e) {
    ...
    aTag_cancel.onclick = function (event) {
        ...
        if(appointmentState==2||appointmentState==5||appointmentState==6){
            ConfirmDialog("آیا مطمئن هستید ؟",patientAppointmentId);
        }else{
             ConfirmDialog("آیا مطمئن هستید ؟",patientAppointmentId);
        }
    };
   ...
   // my  h3_doctorName.innerHTML="دکتر "+doctorName;
    h3_doctorName.setAttribute('style', 'margin-bottom: 0px;color:blue');
   // my   h3_doctorName.id="h3_doctorName_ID_"+appointmentId;
  ...
  //      d5.appendChild(aTag_reg);
    ...
    today=new Date();
    var compare=compareWithToday(appGlobalDate,today);
    if(appointmentState!=2 && appointmentState!=10 && appointmentState!=8 && compare!=-1){
     	var doctorId = $("#doctorCombo").val();
   	 	var departmentId = $("#departmentCombo").val();
    ...
    var aTag_log = document.createElement("a");
    aTag_log.className = "p-kanban-pin uk-panel-badge";
    aTag_log.href = "#";
    aTag_log.setAttribute('style', 'margin-top: 30px');
    aTag_log.innerHTML='مباهده سوابق تغییرات';
    aTag_log.onclick = function (event) {
        event.stopPropagation();
        showLog(patientAppointmentId, patientNF);
    };
	d5.appendChild(aTag_log);
    ...
    if(appointmentState==10){
    	...
    	d3.setAttribute('style', 'background-color:#FFD4D8');
    	
        var h3_state = document.createElement("label");
        h3_state.className ="uk-panel-title";
        h3_state.innerHTML="&nbsp&nbsp" + "  انصراف داده شده" + "&nbsp&nbsp";
        h3_state.setAttribute('style', 'margin-bottom: 0px;margin-top:0px;color:blue');
...
//        d3.appendChild(h3_space);
    ...
    var c2TimeInDayElement = document.getElementById("c2_"+timeInDayId);
    if(c2TimeInDayElement != null){
    	c2TimeInDayElement.appendChild(d1);
    }
	var showCancel = $('#showCancelCheckbox')[0].checked;
    if(appointmentState==6 || appointmentState==7 || appointmentState==2 || appointmentState==10 || appointmentState==8 || showCancel==true){
        $( "#"+patientAppointmentId ).sortable({
            cancel: ".ui-state-disabled"
        });
        $( "#h3_ID_"+patientAppointmentId ).sortable({
            connectWith: ".connectedSortable",
...
function makeCardAndAddForWeek(clkTd,startTime,appointmentId,state){
    var h3 = document.createElement("label");
    h3.id="h3_wv_"+appointmentId;
    h3.appendChild(document.createTextNode(startTime));
    var d3 = document.createElement("div");
    //d3.className ="uk-panel uk-panel-box connectedSortable";
    d3.className ="connectedSortable";
    d3.setAttribute('style', 'border: 2px solid #b1b1b1;padding: 3px 3px;background: #cccccc;width: 90%;border-radius: 15px;');
    d3.style.direction='ltr';
    d3.appendChild(h3);
 	d3.id="div_wv_"+appointmentId;
    
    var sortableStr="";
    var showCancel = $('#showCancelCheckbox')[0].checked;
    $.getJSON("<c:url value='/' />" + "clinic/appointment/appointmentNew/loadPatientAppontment/" + appointmentId + "/" + showCancel, function(dayAppointmentData) {
     	for ( var i =0; i<dayAppointmentData.length;i++) {
     	    if(dayAppointmentData[i].empty != true){
     	    	var h3_name = document.createElement("h3");
    			h3_name.className ="uk-panel-title";
    			h3_name.innerHTML=dayAppointmentData[i].nameAndFam;
    			h3_name.setAttribute('style', 'margin-bottom: 0px');
    			h3_name.id="h3_ID_"+dayAppointmentData[i].id;

				var innerDiv = document.createElement("div");
    			innerDiv.appendChild(h3_name);
    			innerDiv.setAttribute('style', 'border: 2px solid #a1a1a1;padding: 3px 3px;background: #dddddd;width: 90%;border-radius: 25px;');
	    		innerDiv.id=dayAppointmentData[i].id;
    			sortableStr='#'+dayAppointmentData[i].id; 
...
function ConfirmDialog(message,patientAppointmentId){
    $('<div></div>').appendTo('body')
            .html('<div><h6>'+message+'</h6></div>')
            .dialog({
                modal: false, title: 'لغو قرار', zIndex: 10000, autoOpen: true,
			    show: {
			        effect: "slide",
			        duration: 400
			    },
                ...
                buttons: {
                    بلی: function () {
                        $.ajax({
                            url: "${pageContext.request.contextPath}"+"/clinic/appointment/appointmentNew/cancelAppointment/"+patientAppointmentId  ,
                            type:"POST",
                            data:JSON.stringify($('#nameAndFam').val()),
                            dataType:"json",
                            contentType:"application/json; charset=utf-8",
...
function autoCompleteForSearchOnAppointment(date){
    $( "#nameAndFamForAppointment" ).autocomplete({
        source: function( request, response ) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}"+"/clinic/appointment/appointmentNew/autoCompelete/loadSearchPatient/"+date +'/'+$("#doctorCombo").val()+'/'+$("#departmentCombo").val(),
                type:"POST",
...
function autoCompleteForSearchOnAppointmentForWeek(satDate,sunDate,monDate,tueDate,wedDate,thuDate,friDate){
    $( "#nameAndFamForAppointment" ).autocomplete({
        source: function( request, response ) {
            $.ajax({
                url: "${pageContext.request.contextPath}"+"/clinic/appointment/appointmentNew/autoCompelete/loadSearchPatientWeek/"+$("#doctorCombo").val()+'/'+$("#departmentCombo").val()+"/"+satDate+"/"+sunDate+"/"+monDate+"/"+tueDate+"/"+wedDate+"/"+thuDate+"/"+friDate,
                type:"POST",
                data:JSON.stringify($('#nameAndFamForAppointment').val()),
                dataType:"json",
                contentType:"application/json; charset=utf-8",
...
//                console.log(ui.item.id);
            ...
            currentCardSelectedInSearchForWeek=$("#"+ui.item.id);
//                var allData= new Array();
//                allData= ui.item.id.split ('-');
//                $('#fileNumber').val(allData[0]);
//                $('#patientId').val(allData[1]);
...
function showCancelCheckboxChange (){
	var showCancel = $('#showCancelCheckbox')[0].checked;
	if(!showCancel){
		reLoadOnDoctorCombo();
		return;
    ...
    $('<div></div>').appendTo('body')
            .html('<div><h6>در حالت نمایش قرار ویزیت های انصراف داده شده امکان جابجایی نوبت وجود ندارد، آیا موافق هستید؟</h6></div>')
            .dialog({
                modal: false, title: '', zIndex: 10000, autoOpen: true,
			    show: {
			        effect: "slide",
			        duration: 400
			    },
                ...
                buttons: {
                    ...
                    خیر: function () {
                    	$('#showCancelCheckbox')[0].checked = false;
                        $(this).dialog("close");
                    }
                },
                close: function (event, ui) {
...
function showLog(patientAppointmentId, patientName){
	$.getJSON("<c:url value='/' />" + 'clinic/appointment/appointmentNew/loadLog/' + patientAppointmentId, function(data) {
		var grid = "<style>.logTable table {border-collapse: collapse; border: 1px solid black; text-align: center;}.logTable td {border: 1px solid black; padding: 4px;}</style>" 
			+"<table><tr style='color: red;'><td>ردیف</td><td>تاریخ و زمان</td><td>کاربر</td><td>وضعیت قبلی</td><td>وضعیت جدید</td><td>شماره نوبت قبلی</td><td>شماره نوبت جدید</td><td>نوع عملیات</td></tr>";
		for(i=0; i<data.length; i++){
			grid = grid + "<tr><td>"+data[i].rowNumber+"</td><td>"+data[i].logMilliSecondText+"</td><td>"+data[i].user
					+"</td><td>"+data[i].oldState+"</td><td>"+data[i].newState+"</td><td>"+data[i].oldTurn+"</td><td>"+data[i].newTurn
					+"</td><td>"+data[i].actionText+"</td></tr>";
		}
		grid = grid + "</table>";
    	$('<div></div>').appendTo('body')
            .html('<div class="logTable">'+grid+'</div>')
            .dialog({
                modal: false, title: 'مشاهده تغییرات' + ' ('+patientName+') ', zIndex: 10000, autoOpen: true,
			    show: {
			        effect: "slide",
			        duration: 400
			    },
