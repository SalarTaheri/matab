...
<div class="body-header">
    ...
    <div id="header-toolbar">
        <base:buttonPermission key="clinic.patient.visitQueue.refresh" classNames="uk-button uk-button-success"
                               id="visitDoctorQueueView-refresh-btn" onClick="javascript:refreshClick()"/>

        <base:buttonPermission key="clinic.patient.visitQueue.convertVisitPenImage" classNames="uk-button uk-button-danger"
                               id="visitDoctorQueueView-convertVisitPenImage-btn" onClick="javascript:startConvertVisitPenImage()"/>

        <base:buttonPermission key="clinic.patient.visitQueue.convertGhanaeePenImage" classNames="uk-button uk-button-danger"
                               id="visitDoctorQueueView-convertGhanaeePenImages-btn" onClick="javascript:startConvertGhanaeePenImage()"/>

        <base:buttonPermission key="clinic.patient.visitQueue.convertMasjedizadeh" classNames="uk-button uk-button-danger"
                               id="visitDoctorQueueView-convertGhanaeePenImages-btn" onClick="javascript:startConvertAnotherSystemData()"/>

        <div id="body-heade-save-div" style="display:none;">
            <base:buttonPermission key="global.crud.save" classNames="uk-button uk-button-success"
                                   id="body-heade-save"/>
            <base:aPermission key="global.crud.cancel" classNames="uk-button uk-button-danger"
                              id="breadcrumb-main-section-cancel"></base:aPermission>
        </div>
    </div>
</div>
...
<base:mehrSys-grid-comboAndDateWithoutButton isPopup="true" name="grid" transportReadUrl="${basePath}/grid-read"
                                             ...
                                             comboId="comboId"
                                             modelFieldFilterCombo="state" modelFieldFilterText="searchField"
                                             selectable="single"
                                             comboLabelCaptionKey="his.patient.visitInfo.stateFilter"
                                             hasSearchDate="true"
                                             followFunction="evaluteSum" dateFieldName="visitDate"
                                             selectedIndexDropDown="1"
                                             keyConfirmCheck="his.patient.visitQueue.alert.confirmVisitCheck">
    ...
    <base:my-grid-column title='com.artonis.clinic.basicInfo.service.name' field="serviceName" width="60px"/>
    <base:my-grid-column title='clinic.cash.registerDate' field="visitDate" width="30px"/>
    <base:my-grid-column title='his.patient.visitQueue.state' isFlag="true"
                         flagIconUrl="/resources/images/black/button-check.png" flagDestinationUrl="DoVisitUnvisit"
                         field="visitUnvisit" width="25px"/>
    ...
    <base:my-grid-function text="clinic.patient.neurology" functionClick="grid_onNeurology" name="neurology"/>
    <base:my-grid-function text="clinic.patient.visit" functionClick="grid_onPenVisit" name="penVisit"/>
    <%--<base:my-grid-function text="clinic.patient.visit" functionClick="grid_onConfigurablePatientRecord" name="penVisit"/>--%>
...
<div id="aggregateAccordion" style="border: 1">
    ...
    <%--<div id="search_back">--%>
    <div class="uk-grid uk-grid-preserve">
        <base:my-grid name="totalGridAggregate" transportReadUrl="${basePath}/grid-read-totalgrid-aggregate"
                      showOperation="false">

            <base:my-grid-column title='his.patient.visitQueue.patientCount' field="patientCount" width="20px"/>
            <base:my-grid-column title='his.patient.visitQueue.serviceItemName' field="service_item_name"
                                 width="120px"/>
...
//        console.log('mongoVisit_id:'+data.mongoVisit_id);
    ...
    function grid_onNeurology(e) {
        var data = this.dataItem($(e.target).closest("tr"));
        window.location = "${pageContext.request.contextPath}" + "/clinic/patient/neurology/" + data.patient_id + "/" + data.id + "/" + data.mongoVisit_id;
    }
...
/*    function grid_onConfigurablePatientRecord(e){
        var data = this.dataItem($(e.target).closest("tr"));
    ...
    function startConvertVisitPenImage() {
        $.getJSON(basePath + 'startConvertVisitPenImage', function (result) {
            if (result.state == "success") {
                alertify.success(result.message);
                console.log("Finish convert visit pen's images");
            } else {
                alertify.error("An Error Occurred In Convert");
                console.log("An Error Occurred In Convert");
            }
        });
    }

    function startConvertGhanaeePenImage() {
        $.getJSON(basePath + 'startConvertGhanaeePenImage', function (result) {
            if (result.state == "success") {
                alertify.success(result.message);
                console.log("Finish convert Ghanaee pen's images");
            } else {
                alertify.error("An Error Occurred In Convert");
                console.log("An Error Occurred In Convert --Ghanaee pen's images--");
            }
        });
    }

    function startConvertAnotherSystemData() {
        $.getJSON(basePath + 'startConvertAnotherSystemData', function (result) {
            if (result.state == "success") {
                alertify.success(result.message);
                console.log("Finish convert MasjedizadehData");
            } else {
                alertify.error("An Error Occurred In Convert");
                console.log("An Error Occurred In Convert --MasjedizadehData--");
            }
        });
    }
