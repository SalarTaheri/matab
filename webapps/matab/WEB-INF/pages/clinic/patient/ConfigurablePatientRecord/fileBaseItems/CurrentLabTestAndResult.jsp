...
<div>
    ...
    <div id="dialog-f" title='select lab test' style="display:none;direction: ltr;height: 100% !important;">
        <form class="toPopup" style="direction: ltr">
            <input type="text" id="date-current-labtest" value="<%=curStringDate%>" class="uk-width-1-1 currentLabtestDatePicker" style="width: 100px; text-align: center;"/>
            <%
                int rowPerColumn = 10;
                int column = 5;
...
<script>
    ...
    function activeCurrentLabtestDatePicker() {
        $(".currentLabtestDatePicker:not(.pdp-el)").persianDatepicker({
            onShow: function () {
                setTimeout(function () {
                ...
                if (data.saveMedicalTest != undefined) {
                    ...
                    if (split.length > 1) {
                        for (var i = 0; i < split.length - 1; i = i + 2) {
                            rowNumber++;
                            $('#table').append('<tr id="' + split[i] +'-'+rowNumber +'-row' + '"></td>' +
                            '<td contente="true">' + split[i + 1] + '</td>' +
                            '<td contente="true"> <textarea onkeyup="textArea_onKeyUp(this)" id="' + split[i] +'-'+rowNumber+ '" style=" width: 100%;resize:none; "/></td>' +
                            '<td contente="true"><input type="text" id="'+split[i]+'-'+rowNumber+'-labtest-current-date" value="<%=curStringDate%>" class="uk-width-1-1 currentLabtestDatePicker" style="width: 100px; text-align: center;"/></td>' +
                            '<td><input type="button" value="delete" class="uk-button-mini uk-button-danger" onclick="deleteClick(this)" /></td>' +
                            '<td><input type="checkbox" id="'+split[i]+'-'+rowNumber+'-currentLabtest-checkbox-highlight'+'"</td></tr>' + '');
...
<%
 ...
 if(!(request.getAttribute("labTestResultWithValueList") instanceof String) && request.getAttribute("labTestResultWithValueList")!=null){
    ...
    for (DBObject dbObject : labTestResultWithValueList) {
           ...
           %>
            rowNumber++;
            var medicaltestId = '<%=labTestId%>';
            var medicaltestname = '<%=labTestName%>';
            var labTestIsHighlight = '<%=labTestIsHighlight%>';
            var labTestDate = '<%=labTestDate%>';
            selectCurrentLabTestArrayElement[selectCurrentLabTestArrayElement.length]=medicaltestId;
            var value = '<%=StringEscapeUtils.escapeEcmaScript(labTestValue)%>';
            $('#table').append('<tr id="' + medicaltestId +'-'+rowNumber +'-row' + '"></td>' +
            '<td contente="true">' + medicaltestname + '</td>' +
            '<td contente="true"> <textarea onkeyup="textArea_onKeyUp(this)" id="' + medicaltestId +'-'+rowNumber+ '" style=" width: 100%;resize: none; "><%=StringEscapeUtils.escapeEcmaScript(labTestValue)%></textarea></td><td contente="true"><input type="text" id="'+medicaltestId+'-'+rowNumber+'-labtest-current-date" value="'+labTestDate+'" class="uk-width-1-1 currentLabtestDatePicker" style="width: 100px; text-align: center;"/></td><td><input type="button" value="delete" class="uk-button-mini uk-button-danger" onclick="deleteClick(this)" /></td><td><input type="checkbox" id="'+medicaltestId+'-'+rowNumber+'-currentLabtest-checkbox-highlight'+'"</td></tr>' + '');
            if(labTestIsHighlight=='true'){
                $('#'+medicaltestId+'-'+rowNumber+'-currentLabtest-checkbox-highlight').prop('checked',true)
            }
...
$(document).ready(function(){
    activeCurrentLabtestDatePicker();
        ...
        function (e) {
            if (
                    jQuery('#dialog-f').dialog('isOpen')
                            ...
                            && !((jQuery(e.target).parent().parent().parent()[0].className=='pdp-default')
                            || (jQuery(e.target).parent().parent()[0].className=='pdp-default')
                            || (jQuery(e.target).parent()[0].className=='pdp-default'))
                    ) {
...
//        }
        rowNumber++;
        if ( arrayCheckedLabTest[i].id != 'all-select-c-labtest-checkbox') {
            ...
            if(textBoxValue.trim().length>0){
                $('#table').append('<tr id="' + arrayCheckedLabTest[i].id.split('-current-labtest-checkbox')[0] +'-'+rowNumber +'-row' + '"></td>' +
                '<td contente="true">' + arrayCheckedLabTest[i].name + '</td><td contente="true"> <textarea onkeyup="textArea_onKeyUp(this)" style="width:100%;resize: none;" id="' + arrayCheckedLabTest[i].id.split('-current-labtest-checkbox')[0] +'-'+rowNumber+ '" style=" width: 100%; ">'+textBoxValue+'</textarea></td>' +
                '<td contente="true"><input type="text" id="'+arrayCheckedLabTest[i].id.split('-current-labtest-checkbox')[0]+'-'+rowNumber+'-labtest-current-date" value="<%=curStringDate%>" class="uk-width-1-1 currentLabtestDatePicker" style="width: 100px; text-align: center;"/></td>' +
                '<td><input type="button" value="delete" class="uk-button-mini uk-button-danger" onclick="deleteClick(this)" /></td>' +
                '<td><input type="checkbox" id="'+arrayCheckedLabTest[i].id.split('-current-labtest-checkbox')[0]+'-'+rowNumber+'-currentLabtest-checkbox-highlight'+'"</td></tr>' + '');
                $('#'+arrayCheckedLabTest[i].id.split('-current-labtest-checkbox')[0]+'-'+rowNumber+'-labtest-current-date').val($('#date-current-labtest').val());
                activeCurrentLabtestDatePicker();
            }
        }
    }
...
function addClick() {
    ...
    var arrayCheckedLabTest = $("#dialog-f").find(":checkbox");
    for (var i = 0; i < arrayCheckedLabTest.length; i++) {
//        $("#" + currentLabTestWithValueArray[j].id + "-current-labtest-value").val();
        var labtestId=arrayCheckedLabTest[i].id.split("-")[0];
        $("#" +labtestId+"-current-labtest-value").val("");
        arrayCheckedLabTest[i].checked = false;
    }
...
function addLabTestToTableAndSelectCheckBox(labTestId, labTestText) {
    ...
    rowNumber++;
    if (found == false) {
        $('#table').append('<tr id="' + labTestId + '-'+rowNumber+'-row' + '"></td>' +
        '<td contente="true">' + labTestText + '</td>' +
        '<td contente="true"> <textarea  onkeyup="textArea_onKeyUp(this)" id="' + labTestId +'-'+rowNumber+ '" style=" width: 100%;resize: none; "/></td>' +
        '<td contente="true"><input type="text" id="'+labTestId+'-'+rowNumber+'-labtest-current-date" value="<%=curStringDate%>" class="uk-width-1-1 currentLabtestDatePicker" style="width: 100px; text-align: center;"/></td>' +
        '<td><input type="button" value="delete"  class="uk-button-mini uk-button-danger" onclick="deleteClick(this)" /></td>' +
        '<td><input type="checkbox" id="'+labTestId+'-'+rowNumber+'-currentLabtest-checkbox-highlight'+'"</td></tr>' + '');
    }
...
//            $("#table").parent().find("input:text")[0].value
