...
<div id="dialog-datePicker" title='Select Date' style="display:none; direction: ltr; height: 100% !important;">
    <form class="toPopup" style="direction: ltr">
        <table align="center" width="100%" style="margin-top: 20px;">
            <tr>
                <td>
                    <div>
                        <input type="hidden" id="operation-yearValue">
                        <label>Year: </label>
                        <select id="operation-yearCombobox" style="width: 80px; padding: 2px; border-radius: 4px;"></select>
                    </div>
                </td>
                <td>
                    <div>
                        <input type="hidden" id="operation-monthValue">
                        <label>Month: </label>
                        <select id="operation-monthCombobox" style="padding: 2px; border-radius: 4px;">
...
<%} %>
*/
	$('#dialog-current-operation input[groupOperationName=' + e.name + ']').prop('checked', e.checked);
}
                ...
                if(data.saveOperation!=undefined){
                    ...
                    if (split.length > 1) {
                        for (var i = 0; i < split.length - 1; i = i + 2) {
                            ...
                            $('#tableCurrentOperation').append('<tr id="' + split[i] + '-operation-current-checkbox-rowOperation' + '"></td>' +
                                    '<td contente="true">' + split[i+1] + '</td><td contente="true"><textarea rows="10"  id="'+split[i]+'-operation-current'+'" class="operation-description" style="overflow-y: auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                                    '<td contente="true"><input type="text" id="'+split[i]+'-operation-current-date'+'"  class="uk-width-1-1 datepickerYearMonth" style="width: 100px; text-align: center;"/></td>' +
                                    '<td contente="true"><textarea rows="10"  id="'+split[i]+'-operation-current-pathology' +'" class="pathology-description" style="overflow-y: width:100%; auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                                    '<td><input type="button" value="delete" class="uk-button uk-button-danger" onclick="deleteCurrentOperationClick(this)" /></td>' +
                                    '<td><input type="checkbox" id="'+split[i]+'-currentOperation-checkbox-highlight'+'"</td></tr>' + '<input type="hidden" id="'+split[i]+'-operation-current-date-val'+'"/>');
                            $('#'+split[i]+'-operation-current-date').attr('readonly', 'true');
                            autoCompleteCurrentOperationApply(split[i]+'-operation-current');
                            autoCompleteCurrentOperationPathologyApply(split[i]+'-operation-current');
                            //
                            activeDatePicker();
                            //
    ...
    elem.textcomplete([{
        ...
        search: function (term, callback) {
            Common.postJSON("${pageContext.request.contextPath}" +
                    "/clinic/patient/visitMobileBase/operationAutoComplete/" + operationName, JSON.stringify({token:term}),
                    function (data) {
                        var tokens = [];
                        tokens.push(term);
...
function autoCompleteCurrentOperationPathologyApply(id){
    var elem = $("[id='"+ id+"']");
    var operationName=$("#"+elem[0].id+'-checkbox-rowOperation').find('td')[0].innerHTML;

    var pathologyElem = $("[id='"+ id+"-pathology']");
    pathologyElem.textcomplete([{
        match: /(\s|\n)(\w{1,}|[\u0600-\u06FF]{1,})$/,
        search: function (term, callback) {
            Common.postJSON("${pageContext.request.contextPath}" +
                    "/clinic/patient/visitMobileBase/operationPathologyAutoComplete/" + operationName, JSON.stringify({token:term}),
                    function (data) {
                        var tokens = [];
                        tokens.push(term);
                        for (var j = 0; j < data.length; j++)
...
$(document).ready(function(){
    ...
    var operationPathology = '<%=operationPathology%>';
    var operationDateVal = '<%=operationDateVal%>';
    $('#tableCurrentOperation').append('<tr id="' + operationId + '-operation-current-checkbox-rowOperation' + '"></td>' +
            '<td contente="true">' + operationName + '</td>' +
            '<td contente="true"> <textarea rows="10"  id="'+operationId+'-operation-current'+'" class="operation-description" style="overflow-y: auto;overflow-x:auto; width: 100%;height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
            <%--'<td contente="true"><input type="text" id="'+operationId+'-operation-current-date" value="<%=curStringDate%>" class="uk-width-1-1 operationDatePicker" style="width: 100px; text-align: center;"/></td>' +--%>
            '<td contente="true"><input type="text" id="'+operationId+'-operation-current-date'+'"  class="uk-width-1-1 datepickerYearMonth" style="width: 100px; text-align: center;"/></td>' +
            '<td contente="true"><textarea rows="10"  id="'+operationId+'-operation-current-pathology' +'" class="pathology-description" style="overflow-y: width:100%; auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
            '<td><input type="button" value="delete" class="uk-button uk-button-danger" onclick="deleteCurrentOperationClick(this)" /></td>' +
            '<td><input type="checkbox" id="'+operationId+'-currentOperation-checkbox-highlight'+'"</td></tr>' + '<input type="hidden" id="'+operationId+'-operation-current-date-val'+'"/>');
    $('#'+operationId+'-operation-current-date').attr('readonly', 'true');
	autoCompleteCurrentOperationApply(operationId+'-operation-current');
    autoCompleteCurrentOperationPathologyApply(operationId+'-operation-current');
    //
    activeDatePicker();
    //
    $('#'+operationId+'-operation-current').val(operationVal);
    $('#'+operationId+'-operation-current-pathology').val(operationPathology);
    $('#'+operationId+'-operation-current-date').val(operationDate);
    $('#'+operationId+'-operation-current-date-val').val(operationDateVal)
    ...
    $("#dialog-datePicker").dialog({
        ...
        buttons: {
            "ok": function (e) {
                ...
                makeDate($(this).data('e').currentTarget.id);
                $(this).dialog("close");
            },
            "cancel": function () {
                $(this).dialog("close");
        ...
        open: function (event, ui) {
            checkForLoadOperationYearAndMonth($(this).data('e').currentTarget.id);
        }
    });
    //
    //
    operationFillYearCombo();
    //
    checkForLoadOperationYearAndMonth();
});

    function makeDate(content){
        var year = $("#operation-yearCombobox").val();
        var month = $("#operation-monthCombobox").val();
        if ($.isNumeric(year) && $.isNumeric(month)) {
            $("#operation-yearValue").val(year);
            $("#operation-monthValue").val(month);
            //
            $("#"+content).val($("#operation-monthCombobox option:selected").text() + " " + year);
            $("#"+content+"-val").val(year+"-"+month);
        }
    }

    function operationFillYearCombo(){
        for (var i = 1390; i<=1450 ; i++) {
            $("#operation-yearCombobox").append("<option>" + i + "</option>");
        }
    }

    function checkForLoadOperationYearAndMonth(content){
        var year;
        var month;
        if ($("#"+content+"-val").val() != undefined && $("#"+content+"-val").val() != "" && $("#"+content+"-val").val().trim().length > 0) {
            year = parseInt($("#"+content+"-val").val().split("-")[0]);
            month = parseInt($("#"+content+"-val").val().split("-")[1]);
        } else {
        ...
        $("#operation-yearValue").val(year);
        $("#operation-monthValue").val(month);
        $("#operation-yearCombobox").val(year);
        $("#operation-monthCombobox").val(month);
    }
...
//                currentOperationpreload_data.push({ComboID:addId,ComboCaption:deleteId})
        ...
        function addOperationToTableAndSelectCheckBox(operationId,operationText){
            ...
            if(found==false){
                $('#tableCurrentOperation').append('<tr id="' + operationId + '-operation-current-checkbox-rowOperation' + '"></td>' +
                        '<td contente="true">' + operationText + '</td>' +
                        '<td contente="true"><textarea rows="10"  id="'+operationId+'-operation-current' +'" class="operation-description" style="overflow-y: width:100%; auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                        '<td contente="true"><input type="text" id="'+operationId+'-operation-current-date'+'"  class="uk-width-1-1 datepickerYearMonth" style="width: 100px; text-align: center;"/></td>' +
                        '<td contente="true"><textarea rows="10"  id="'+operationId+'-operation-current-pathology' +'" class="pathology-description" style="overflow-y: width:100%; auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                        '<td><input type="button" value="delete" class="uk-button uk-button-danger" onclick="deleteCurrentOperationClick(this)" /></td>' +
                        '<td><input type="checkbox" id="'+operationId+'-currentOperation-checkbox-highlight'+'"</td></tr>' + '<input type="hidden" id="'+operationId+'-operation-current-date-val'+'"/>');
        ...
        function activeDatePicker(){
            /*$(".operationDatePicker:not(.pdp-el)").persianDatepicker({
                onShow: function() {
                    setTimeout(function(){
            ...
            $('.datepickerYearMonth').on('click', function(e){
                $("#operation-yearCombobox").val($("#operation-yearValue").val());
                $("#operation-monthCombobox").val($("#operation-monthValue").val());
                $("#dialog-datePicker").data('e',e).dialog("open");
            });
        }
        ...
        function popupCurrentOperationOkButton(){
            ...
            for (var i = 0; i < arrayCheckedOperation.length; i++) {
                ...
                if(found==false){
                    $('#tableCurrentOperation').append('<tr id="' + arrayCheckedOperation[i].id + '-rowOperation' + '"></td>' +
                            '<td contente="true">' + arrayCheckedOperation[i].name + '</td>' +
                            '<td contente="true"><textarea rows="10"  id="'+arrayCheckedOperation[i].id.split("-checkbox")[0] +'" class="operation-description" style="overflow-y: auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                            <%--'<td contente="true"><input type="text" id="'+arrayCheckedOperation[i].id.split("-checkbox")[0] +'-date" value="<%=curStringDate%>" class="uk-width-1-1 operationDatePicker" style="width: 100px; text-align: center;"/></td>' +--%>
                            '<td contente="true"><input type="text" id="'+arrayCheckedOperation[i].id.split("-checkbox")[0]+'-date'+'"  class="uk-width-1-1 datepickerYearMonth" style="width: 100px; text-align: center;"/></td>' +
                            '<td contente="true"><textarea rows="10"  id="'+arrayCheckedOperation[i].id.split("-checkbox")[0] +'-pathology' +'" class="pathology-description" style="overflow-y: width:100%; auto;overflow-x:auto; width: 100%; height:50px !important;   direction: ltr; text-align: left"  >&nbsp;</textarea></td>' +
                            '<td><input type="button" value="delete" class="uk-button uk-button-danger" onclick="deleteCurrentOperationClick(this)" /></td>' +
                            '<td><input type="checkbox" id="'+arrayCheckedOperation[i].id.split("-operation-current-checkbox")[0]+'-currentOperation-checkbox-highlight'+'"</td></tr>' + '<input type="hidden" id="'+arrayCheckedOperation[i].id.split("-checkbox")[0]+'-date-val'+'"/>');
                    $('#'+arrayCheckedOperation[i].id.split("-checkbox")[0]+'-date').attr('readonly', 'true');
                    autoCompleteCurrentOperationApply(arrayCheckedOperation[i].id.split("-checkbox")[0]);
                    autoCompleteCurrentOperationPathologyApply(arrayCheckedOperation[i].id.split("-checkbox")[0]);
