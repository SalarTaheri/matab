...
<head>
    ...
    <script src="<c:url value='/resources/sweetalert/sweetalert.min.js'/>"></script><link href="<c:url value='/resources/clinic/mergedClinicCss.css'/>" rel="stylesheet" /></head>
</head>
<body>
		...
		<td width="60%" dir="ltr" align="left" height="100%">
			<div class="body-header">
				<div id="header-toolbar">
                    ...
                    <button class="uk-button uk-button-success" id="printSurgery"
                            type="button">Print
                    </button>
				</div>
			</div>
            ...
            <div class="uk-grid uk-grid-preserve">
                <base:multiSelect serverFilter="true" name="rvu_ids" styleClass="small" loadUrlIsAbsolute="true"
                                  multiSelectLoaderUrl="/clinic/patient/doctorSurgery/combo/allOperatingType" key="clinic.patient.surgery.kaCode" />
                <%--<base:textArea name="address" key="clinic.patient.surgery.kaCodeDesc" height="60" disabled="true"/>--%>
            </div>
			<div id="surgeryExplaination" style="height: 100%">
                ...
                <div style="background-color: white; border-style: solid; border-color: black; border-width: 1px; display: flex; flex-direction: column; ">
                    ...
                    <div style="text-align: right; padding: 5px; display: flex; direction: rtl; padding-bottom: 10px;">
                        <base:multiSelect serverFilter="true" name="templateCombo" styleClass="normal" multiSelectLoaderUrl="/clinic/patient/doctorSurgery/combo/loadAllTemplate" stylediv="width: 90%;"
                                          key="clinic.patient.radiology.radiologyTemplate" styledivLabel="width: 120px; text-align: right;" isBoundInOpen="true" loadUrlIsAbsolute="true"/>
                    </div>
                </div>
                <div id="surgeryTab" style="display: flex; flex-direction: row; flex-wrap: wrap; justify-content: center; align-items: center;height:30px; background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,rgba(30,87,153,1)), color-stop(50%,rgba(41,137,216,1)), color-stop(51%,rgba(32,124,202,1)), color-stop(100%,rgba(125,185,232,1)));">
                 	<div style="color: white; display: table; margin: 0 auto;">
                 		<%=ResourceBoundleHolder.resourceBundle.getString("clinic.patient.surgery.explain") %>
                	</div>
                </div>
                <div style="background-color: white; border-style: solid; border-color: black; border-width: 1px; height: 500px;">
                	<%--<kendo:editor name="surgeryExplanation" id="surgeryExplanation" class="uk-width-1-1" />--%>
                        <textarea name="surgeryExplanation" id="surgeryExplanation" ></textarea>
                </div>
...
//	            }
	...
	$("#printSurgery").click(printSurgeryClick);
    ...
    function isNumber(n) {
  	...
  	function saveFormActual_1(visitId){
		var data = {};
//        data.surgeryExplanation = $("#surgeryExplanation").data("kendoEditor").value();
        data.surgeryExplanation = CKEDITOR.instances.surgeryExplanation.getData();
		data.patientId = "<%=patientId%>";
		data.visitQueueId = "<%=visitQueueId%>"; 
		data.mongoVisitId = "<%=mongoVisitId%>";
        data.rvu_ids=$("#rvu_ids_multiSelect").val();
		$.ajax({
			url: '${pageContext.request.contextPath}'+'/clinic/patient/doctorSurgery/saveSurgeryExplanation',
        ...
        function printSurgeryClick(){
            var myWindow = window.open("", "Surgery", "width=800, height=800");
            var re = CKEDITOR.instances.surgeryExplanation.getData();
        ...
        var surgeryExplanationTextState = 0;

        var surgeryTemplate = "-1";
        ...
        function callback_change_templateCombo(e) {
            ...
            if (e.sender._dataItems.length > 0) {
                ...
                for (var i = 0; i < e.sender._dataItems.length; i++) {
                    surgeryTemplate += e.sender._dataItems[i].ComboID + ":_:";
                }
            }
            changeTemplate();
        }
        ...
        function loadHeaderCombo(pageInEditMode){
            $.ajax({
                url: "${pageContext.request.contextPath}"+"/clinic/patient/doctorSurgery/combo/loadHeaderCombo",
                type:"GET",
                dataType:"json",
                contentType:"application/json; charset=utf-8",
    ...
    $(document).ready(function () {
    	...
    	var visitQueueId = "<%=visitQueueId%>";
        CKEDITOR.replace('surgeryExplanation');
        CKEDITOR.on("instanceReady", function(event){
            $.getJSON( '${pageContext.request.contextPath}'+'/clinic/patient/doctorSurgery/loadSurgeryExplanation/'+mongoVisitId+'/'+visitQueueId, function (data) {
//        	$("#surgeryExplanation").data("kendoEditor").paste(data.surgeryExplanation);
        	    $("#rvu_ids_multiSelect").data("kendoMultiSelect").value(data.rvuIds);
                CKEDITOR.instances.surgeryExplanation.setData(data.surgeryExplanation);
                surgeryExplanationText = data.surgeryExplanation;
                surgeryExplanationTextState = 0;

                var pageEditMode = false;
                if(data.surgeryExplanation != null && data.surgeryExplanation != undefined && data.surgeryExplanation != ""){
                    pageEditMode = true;
    ...
    function    callback_change_rvu_ids(e){
//        var data = {};
        /*var rvu_ids=e.sender._values;
        $.getJSON('${pageContext.request.contextPath}'+'/clinic/patient/doctorSurgery/loadRvu/'+rvu_ids, function (data) {
        ...
        function fillHeaderAndTemplateSelectorItems(){
            var data = CKEDITOR.instances.surgeryExplanation.getData();
        ...
        function loadSurgeryExplanationEditor(){
            var templateComboValue = surgeryTemplate;
            ...
            //
            var url = '${pageContext.request.contextPath}'+'/clinic/patient/doctorSurgery/loadSurgeryExplanationFromCombo/' +
                templateComboValue + '/' + templateHeaderValue + '/' + patientId;
            //
            $.getJSON(url, function (data) {
                var oldData = CKEDITOR.instances.surgeryExplanation.getData().replace(/(\r\n|\n|\r)/gm,"");;
                var newData = data.surgeryExplanation;

                if (oldData == "") {
                    CKEDITOR.instances.surgeryExplanation.setData(newData);
                }
