...
<div class="uk-width-1-1" id="drug-kanban-static" style=" height:100%; margin-left: 5px; margin-top: 10px;">
    ...
    <div>
        <button name="Adddiagnosis" id="addCurrentdiagnosis" class="uk-button-success uk-active"
                style="margin-top:10px;height:29px" onclick="adddiagnosisClick()"
                title="Adddiagnosis">Add Diagnosis
        </button>
    </div>
    <div id="tableCurrentDiagnosisDiv" style=" padding-top: 10px; ">
        <table id="tablediagnosisDesc" height="100%" width="100%" style="text-align: center;background-color: #ffffff;"
               border="2" dir="ltr">
            <tr style=" background: -webkit-linear-gradient(top, rgba(212,225,241,1) 29%,rgba(201,218,238,1) 33%,rgba(228,240,253,1) 100%); height: 25px; ">
                ...
                <th id="highlightdiagnosis" style="display: none;">
                    Highlight
                </th>
            </tr>
...
<div id="dialog-current-diagnosis" title='select diagnosis'
     style="display:none;direction: ltr;height: 100% !important;">
    <form class="toPopup" style="direction: ltr">
        <%
        ...
        %>
            <table id="dialog-current-diagnosis-table">
                <tr style="background-color: #9cf;">
                    <td>
                        <input type="checkbox" name="<%=diagnosisCustomizationItemView.getName()%>"
                               onclick="groupCurrentDiagnosisSelect(this)" isDiagnosisCurrentGroup="true"
                               id="<%=diagnosisCustomizationItemView.getName()%>-group-current-diagnosis-customize"/>
                        <label for="<%=diagnosisCustomizationItemView.getName()%>-group-current-diagnosis-customize"><span
                                style="font-weight:bold"><%=diagnosisCustomizationItemView.getName()%></span>
                        </label>
                    </td>
                    <td></td>
                    <td></td>
                ...
                <tr id="current-coding-<%=currentCoding%>">
                    <td width="25%">
                        <input type="checkbox" name="<%=diagnosisCustomizationItemView.getDiagnosisName()%>"
                               groupDiagnosisName="<%=diagnosisCustomizationItemView.getName()%>"
                               id="<%=diagnosisCustomizationItemView.getDiagnosisId().toString()%>-diagnosis-current-checkbox"/>
                        <label for="<%=diagnosisCustomizationItemView.getDiagnosisId().toString()%>-diagnosis-current-checkbox"><%=diagnosisCustomizationItemView.getDiagnosisName()%>
                        </label>
                    </td>
                    <%
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        <%}%>
        autoCompleteCurrentdiagnosisApply(diagnosisId + '-diagnosis-rowDiagnosis', diagnosisId + '-diagnosis-textarea');
...
//        var arrayCheckedLabTest = $("#dialog-Diagnosis").find(":checkbox:checked");
            ...
            $.ajax({
                ...
                success: function (data, success, xhr) {
                    if (data.diagnosisId != undefined) {
                        ...
                        afterRemoveRowFromTablediagnosis();
                        autoCompleteCurrentdiagnosisApply(data.diagnosisId + '-diagnosis-rowDiagnosis', data.diagnosisId + '-diagnosis-textarea');
    ...
    function saveDiagnosisItems() {
        ...
        $.ajax({
            url: '${pageContext.request.contextPath}' + '/clinic/patient/penVisit/saveDiagnosis',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(data),
            dataType: 'json',
            type: 'POST',
            success: function (data, success, xhr) {
                ...
                if ($(".ui-tabs-active a").text() == "Diagnosis") {
                    emptyDiagnosis();
                    loadDiagnosis();
                } else {
                    $("a[href='#diagnosis-tab']").click();
                }
            }
        });
    }

    function activeFocusOutFn(){
        $(".diagnosis-description").focusout(function(){
            saveDiagnosisItems();
        });
    }
...
<style type="text/css">
    ...
    #tablediagnosisDesc td:nth-child(4) {
        display: none;
    }
</style>
