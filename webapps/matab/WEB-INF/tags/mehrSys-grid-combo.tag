...
%>
	...
	<div class="k-rtl">	
		<kendo:grid  name="${name}" selectable="${selectable}" pageable="true" sortable="true" height="400"  columnReorder="true" columnResize="true" filterable="true" navigatable="true">
		...
		<div class="toolbar">
	    		...
	    		<kendo:dropDownList name="categories" optionLabel='<%=ResourceBoundleHolder.resourceBundle.getString("global.all") %>' dataTextField="${comboCaption}" 
	    				dataValueField="${comboId}" autoBind="false" change="categoriesChange">
	    			<kendo:dataSource>
	...
	//                                                console.log("document.getElementById:"+document.getElementById("id").value);
	    ...
	    function categoriesChange() {
         ...
         if (value) {
             grid.dataSource.filter({ field: "${modelFieldFilter}", operator: "eq", value: parseInt(value) });
         } else {
