...
<%@ attribute name="modelFieldFilter" required="true" rtexprvalue="true"%>
...
<%@ attribute name="showSearchIcon" required="false" rtexprvalue="true" type="java.lang.Boolean" %>
...
%>
	...
	<div class="k-rtl">	
		<kendo:grid  name="${name}" selectable="${selectable}" pageable="true" sortable="true" height="400"
                     columnReorder="true" columnResize="true" filterable="true" navigatable="true" dataBound="${name}_onDataBound">
        <%--columnReorder="true" columnResize="true" filterable="true" navigatable="true" change="_${name}_onSelectRow">--%>
		<kendo:grid-groupable>
					<kendo:grid-groupable-messages empty='${label_action_groupBy}' />
		</kendo:grid-groupable>
		<kendo:grid-toolbarTemplate>
		<div class="toolbar">
    			<label class="category-label" for="categories"><%=ResourceBoundleHolder.resourceBundle.getString("global.search") %></label>
	    		<input style=" width:18%" type="text" id="categoriesId" name="categories" onkeyup="categoriesChange(event)" >
            <%if(showSearchIcon){%>
	...
	//                                                console.log("document.getElementById:"+document.getElementById("id").value);
	                                                var tmp = {};
        ...
        <script>
			function ${name}_onDataBound(e) {
               ...
               if(typeof(callback_${name}_before_delete) == "function"){
                   var returnValue=callback_${name}_before_delete(this.dataItem($(e.target).closest("tr")));
		...
		function doSearchLikeOnGrid(){
			var value = $(categoriesId).val(),
				 	 grid = $("#${name}").data("kendoGrid");
            if (value) {
		             grid.dataSource.filter({ field: "${modelFieldFilter}", operator: "contains", value: (value) });
	        } else {
		             grid.dataSource.filter({});
	        }
		}

        <%--function _${name}_onSelectRow(e) {--%>
        ...
        $("#${name}").on("dblclick", "tr.k-state-selected", function () {
            var onSelectRowEvent="<%=onDoubleSelectRowEvent%>";
            if(onSelectRowEvent!="null" ){
                var selectedDataItems = [];
                var dataItem = $("#${name}").data("kendoGrid").dataItem(this);
                selectedDataItems.push(dataItem);
                window[onSelectRowEvent](selectedDataItems);
            }
        });
