...
<%if(!isPassword){%>
<div style="<%=divStyle %>" class="<%= styleClass%>">
    <%if(Boolean.valueOf(disabled)){%>
    <input type="text" id="${name}_txt" name="${name}_txt" class="uk-width-1-1" disabled data-ignore="true" onkeypress="return onlyNos(event,this);" onkeyup="addCommas(event,this)<%=onChangeFunctionName%>" data-numericEx="true" <%=readOnly==true?"readonly":""%>>
    <%} else { %>
    <%if(!isMandatory){%>
    <input type="text" id="${name}_txt" name="${name}_txt" class="uk-width-1-1" style="${direction}" data-ignore="true" onkeypress="return onlyNos(event,this);" onkeyup="addCommas(event,this)<%=onChangeFunctionName%>" data-numericEx="true" <%=readOnly==true?"readonly":""%>>
    <%}else {%>
    <input type="text" id="${name}_txt" name="${name}_txt" class="uk-width-1-1" style="background: rgb(206, 215, 235);${direction}" data-ignore="true" onkeypress="return onlyNos(event,this);" onkeyup="addCommas(event,this)<%=onChangeFunctionName%>" data-numericEx="true" <%=readOnly==true?"readonly":""%>>
    <%} %>
    <%} %>
</div>
<%}else {%>
