...
<%@ attribute name="popupUrlIsRelative" required="false" rtexprvalue="true" type="java.lang.Boolean"%>
...
//                 template+="<p>#: data."+templateElement+" #</p>";
        ...
        if(popupUrlIsRelative==null){
            popupUrlIsRelative=true;
        }
    ...
    <kendo:comboBox name="${name}_Comb"  template="<%=template%>" dataTextField="${dataTextField}"
        ...
        <kendo:dataSource serverFiltering="true" pageSize="<%=pageSize%>" serverPaging="true" >
            <kendo:dataSource-transport>
                <kendo:dataSource-transport-read url="<%=s%>" type="POST" contentType="application/json"/>
...
<script type="text/javascript">
    ...
    function add_${name}_Callback(e){
        $("#item-modals-${name}").dialog('close');
        var combobox = $("#${name}_Comb").data("kendoComboBox");
//            combobox.dataSource.query();
//            combobox.refresh();

		var modelCaption = e.modelCaption;
		var modelId = e.modelID.toString().replace("group", "");
        combobox.dataSource.data([{"<%=dataTextField%>":modelCaption, "<%=dataValueField%>":modelId}]);

//        combobox.dataSource.read({
//            filter: { field: "id", operator: "eq", value:e.modelID },
//            page : 1,
//            pageSize : 1
//        });
        ...
        if(typeof(callback_select_element_${name}) == "function"){
            callback_select_element_${name}(modelId);
        }
        if(typeof(callback_select_element_model_${name}) == "function"){
            callback_select_element_model_${name}(e);
        }

    }

    $(document).ready(function() {
        $("#item-modals-${name}").draggable().resizable();
