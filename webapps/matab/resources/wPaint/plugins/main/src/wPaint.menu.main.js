(function ($) {
  ...
  $.fn.wPaint.extend({
    ...
    _drawPencilDown: function (e) {
    	...
    	tmp_ctx.strokeStyle = this.options.strokeStyle;
    	tmp_ctx.fillStyle = this.options.strokeStyle;
    	tmp_ctx.lineWidth = this.options.lineWidth;
    ...
    _drawPencilMove: function (e) {
		...
		if(this.options.mode == "eraser"){
	    	var tmp_canvas = document.querySelector('#tmp_canvas');
			var tmp_ctx = tmp_canvas.getContext('2d');
	    	tmp_ctx.strokeStyle = "#efff00";
	    	tmp_ctx.fillStyle = "#efff00";
		}
    ...
    _drawPencilUp: function () {
		...
		this.ctx.drawImage(tmp_canvas, 0, 0, tmp_canvas.style.width.replace('px',''), tmp_canvas.style.height.replace('px',''));
